...
public class UserController implements HandlerExceptionResolver{
	...
	public Map<String,Object> updateInfo(User user,HttpSession session){
		...
		int flag=cmi.updateUser(user);
		if(flag>0){
			json.put("mess", true);
			user=cmi.queryUserInfo(user.getIdCard());
			UserAllInfo uai=(UserAllInfo) session.getAttribute("uai");
			uai.setUser(user);
			session.setAttribute("uai", uai);
		}else{
	...
	public Map<String, Object> queryTradeRecord(Integer page,
		...
		if(null == lists || lists.size() == 0){
			//如果没有消费记录，提前返回结果
			json.put("noresult", true);
			return json;
		}
		
		//创建一个实际属于该用户的消费记录
		List<TradeRecord> thisList = new ArrayList<>();
		//从所有记录中根据卡号获取属于该用户的消费记录
		for(int i=0; i<lists.size(); i++){
			if(lists.get(i).getCardNum().equals(uai.getCardMessage().getCardNum())){
				thisList.add(lists.get(i));
			}
		}
		...
		//若最后一页没有3条记录，改变结束点。否则会数组下标越界
		int end = 0;
		if (endTest > thisList.size()) {
			end = thisList.size();
		} else {
			end = endTest;
		}
		...
		//显示在页面的消费记录，每次取三条
		List<TradeRecord> trList = thisList.subList(start, end);
