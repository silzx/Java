...
public interface UserDao {
	...
	@describe  查看改身份证用户是否已经办理过信用卡
	*/
	@Select("select count(*) from  carduser where idcard=#{0}")
	int obtainUser(String idcard);
	/** * 
	@author    姚彬彬 
	@date      2018年1月9日 下午4:57:54   
	@describe  插入user表
	*/
	...
	@Update("update carduser c,bills b set c.avabalance=c.avabalance-#{price},c.usedbalance=c.usedbalance+#{price},"
			+ " b.notreadybill=b.notreadybill+#{price} where c.avabalance>#{price} and b.idcard=c.idcard and b.idcard=#{idcard}")
	 ...
	 */
	@Select("SELECT t.SwiftNumber,t.cardNum,t.TradeType,t.Money,t.EnterDate,t.TradeShop,"
	...
	@Results({
		@Result(column="SwiftNumber",property="tradeNum"),
		@Result(column="TradeShop",property="storeName"),
		@Result(column="Money",property="price"),
		@Result(column="EnterDate",property="tradeDate")
	})
