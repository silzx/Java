...
function fuzzyQuery(){
	...
	javascript:location.href="/afterAudit/secondIndex?fuzzy="+fuzzy;
}
</script>
<script type="text/javascript">
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：<a href=""  >信用卡申请审核</a> >>  <a href=""  >二审管理</a></td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
  		<tr>
   		 <td width="90%" align="left" valign="middle">
	         ...
	         <span>二审查询：</span>
	         <input type="text" name="fuzzy" value="" class="text-word">
	         <input name="" type="button" value="查询" onclick="fuzzyQuery();" class="text-but">
	         </form>
         </td>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr>
        ...
        <th align="center" valign="middle">操作</th>
            ...
            <c:forEach  var="item" items="${second_list }">
          ...
          <fmt:formatDate value="${item.applyDate }" pattern="yyyy-MM-dd hh:mm:ss"/>
           ...
           <td align="center" valign="middle" class="borderbottom"><a href="/afterAudit/secondInfo?uuid=<c:out value="${item.uuid }"/>" target="mainFrame" onFocus="this.blur()" class="add">申请详情</a><span class="gray"></td>
         </tr>
  ...
  <tr>
   <td align="left" valign="top" class="fenye"><c:out value="${count }"/>条数据 <c:out value="${cupage }"/>/<c:out value="${pages }"/> 页&nbsp;&nbsp;<a href="/afterAudit/secondIndex?cupage=1&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;<a href="/afterAudit/secondIndex?cupage=<c:out value="${(cupage-1)<=1?1:cupage-1 }"/>&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;<a href="/afterAudit/secondIndex?cupage=<c:out value="${cupage+1>=pages?pages:cupage+1 }"/>&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;<a href="/afterAudit/secondIndex?cupage=<c:out value="${pages }"/>&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">尾页</a></td>
  </tr>
</table>
</body>
</html>