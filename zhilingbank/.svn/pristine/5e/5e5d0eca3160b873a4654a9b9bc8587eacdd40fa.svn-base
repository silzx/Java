...
function fuzzyQuery(){
	...
	javascript:location.href="/afterAudit/rejectedIndex?fuzzy="+fuzzy;
}
</script>
<script type="text/javascript">
...
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：<a href=""  >信用卡申请审核</a> >>  <a href=""  >驳回信息管理</a></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr>
        ...
        <th align="center" valign="middle" class="borderright">意见反馈</th>
      ...
      <c:forEach  var="item" items="${customer_list }">
          <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
           ...
           <td align="center" style="max-width:200px; overflow: hidden; text-overflow:ellipsis"  valign="middle" class="borderright borderbottom"> <c:out value="${item.remark }"/></td>
           <td align="center" valign="middle" class="borderright borderbottom">         
          <fmt:formatDate value="${item.applyDate }" pattern="yyyy-MM-dd hh:mm:ss"/>
           </td>
           <td align="center" valign="middle" class="borderbottom"><a href="/afterAudit/rejectedInfo?uuid=<c:out value="${item.uuid }"/>" target="mainFrame" onFocus="this.blur()" class="add">申请详情</a><span class="gray"></span></td>
         </tr>
  ...
  <tr>
    <td align="left" valign="top" class="fenye"><c:out value="${count }"/>条数据 <c:out value="${cupage }"/>/<c:out value="${pages }"/> 页&nbsp;&nbsp;<a href="/afterAudit/rejectedIndex?cupage=1&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;<a href="/afterAudit/rejectedIndex?cupage=<c:out value="${(cupage-1)<=1?1:cupage-1 }"/>&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;<a href="/afterAudit/rejectedIndex?cupage=<c:out value="${cupage+1>=pages?pages:cupage+1 }"/>&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;<a href="/afterAudit/rejectedIndex?cupage=<c:out value="${pages }"/>&fuzzy=<c:out value="${fuzzy }"/>" target="mainFrame" onFocus="this.blur()">尾页</a></td>
  </tr>
</table>
</body>
</html>